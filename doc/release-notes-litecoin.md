Pakcoin Core version *0.15.1* is now available from:

  <https://download.pakcoin.org/pakcoin-0.15.1/>
...
A number of changes to the way Pakcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 pai per kB (0.00001 PAK). This is to prepare for dropping the minimum transaction fee to 0.1 pai per kB (0.0001 PAK) in the next release.
