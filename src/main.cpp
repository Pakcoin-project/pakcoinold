...
#if defined(NDEBUG)
# error "Pakcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Pakcoin Signed Message:\n";
...
{
    // Pakcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Pakcoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // Pakcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (pindexLast->nTime > 1466640000)
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pakcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x48ce59ea60dec70e478a07e2362f1bade9d7c025072a301121fcf3fd9ff84933");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        if (fTestNet)
        {
            block.nTime    = 1432771260;
            block.nNonce   = 389863888;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x72bb72ee57a0e14a51f80011feb167105bfa40412b4385f54b84e1573b8ede99"));
...
unsigned char pchMessageStart[4] = { 0x70, 0x61, 0x6b, 0x63 }; // Pakcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PakcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PakcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PakcoinMiner(CWallet *pwallet)
{
    printf("PakcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pakcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PakcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PakcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PakcoinMiner, pwallet));
}
